...
class FastTextUnsupervised(BaseModel):
    ...
    def train(self, config):
        ...
        Args:
            ...
            dim (int): Dimension of hidden layer,
                needs to be adjusted depending on :attr:`pretrained_vectors`.
                Default: 100
            ws (int): Context window size. Default: 5
            num_epochs (int): Number of epochs. Default: 5
            ngrams (int): Max length of word ngram. Default: 1
            lr_update_rate (float): Update rate for learning rate. Default: 100
            t (float): Sampling threshold. Default: 0.0001
            pretrained_vectors (string): Path to pretrained model
                (available here: https://github.com/facebookresearch/fastText/blob/master/docs/crawl-vectors.md).
                Default: ``''``
            quantize (bool): Whether to quantize model before saving.
                Default: ``False``
            save_model (bool): Whether to save model in binary format (`.bin`).
                Default: ``True``
            save_vec (bool): Whether to save vectors (`.vec`). Default: ``True``
        """
        ...
        self.output_vectors_path = os.path.join(config.output_path,
                                                'vectors.vec')
        self.label_mapping = self.set_label_mapping(
            config,
            train_data=config.fine_tune_data)
        model_args = {
