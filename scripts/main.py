...
class ArgParse(object):
    ...
    def split(self):
        from text_classification.utils.helpers import train_test_split
    ...
    def train(self):
        ...
        """
        from text_classification.utils.helpers import train
    ...
    def predict(self):
        from text_classification.utils.helpers import predict
        ...
        parser.add_argument('--output-cols', dest='output_cols', required=False, type=str, default='labels,probabilities,label,probability', help="Output columns, provide as comma-separted string. only applies to CSV format.")
        ...
        args = parser.parse_args(sys.argv[2:])
        predict(args.run, path=args.path, data=args.data, output_cols=args.output_cols, output_folder=args.output_folder, col=args.col, no_file_output=args.no_file_output, in_parallel=args.in_parallel, verbose=args.verbose, output_formats=args.output_formats)

    def generate_config(self):
        from text_classification.utils.helpers import generate_config
    ...
    def generate_text(self):
        from text_classification.utils.helpers import generate_text
    ...
    def augment(self):
        from text_classification.utils.helpers import augment_training_data
    ...
    def finetune(self):
        ...
        """
        from text_classification.utils import ConfigReader
        from text_classification.utils.helpers import finetune
        parser = ArgParseDefault(description=desc)
        parser.add_argument('-c', '--config', metavar='C', required=False, default='config.json', help='Name/path of configuration file. Default: config.json')
        args = parser.parse_args(sys.argv[2:])
        config_reader = ConfigReader()
    ...
    def learning_curve(self):
        ...
        """
        from text_classification.utils.helpers import learning_curve
        parser = ArgParseDefault(description=desc)
        parser.add_argument('-c', '--config', metavar='C', required=False, default='config.json', help='Name/path of configuration file. Default: config.json')
        args = parser.parse_args(sys.argv[2:])
    ...
    def optimize(self):
        ...
        """
        from text_classification.utils.helpers import optimize
        parser = ArgParseDefault(description=desc)
        parser.add_argument('-c', '--config', metavar='C', required=False, default='config.json', help='Name/path of configuration file. Default: config.json')
        args = parser.parse_args(sys.argv[2:])
    ...
    def deploy(self):
        ...
        """
        from text_classification.utils.deploy_helpers import deploy
